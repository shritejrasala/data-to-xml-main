...
function referencedType(context, ref) {
    const expectedPrefix = context.v3 ? "#/components/schemas/" : "#/definitions/";
...
function structElements(context, type, schema) {
    ...
    const schemaName = Object.keys(context.schemas).find(key => context.schemas[key] === schema);
    ...
    for (const [prop, propSchema] of Object.entries(schema.properties || {})) {
        ...
        if ((propSchema?.$ref === refPath) || (propSchema?.items?.$ref === refPath)) {
            console.warn(`Recursive data type detected: ${schemaName} for the property ${name}`);
        }
...
function checkCircularReference(schema, visited = new Set()) {
    ...
    if (schema.allOf) {
        schema.allOf.forEach(subSchema => checkCircularReference(subSchema, visited));
    }
    
    if (schema.anyOf) {
        schema.anyOf.forEach(subSchema => checkCircularReference(subSchema, visited));
    }
    
    if (schema.oneOf) {
        schema.oneOf.forEach(subSchema => checkCircularReference(subSchema, visited));
    }
    
    if (schema.properties) {
        Object.values(schema.properties).forEach(subSchema => checkCircularReference(subSchema, visited));
    }
