...
function _initialize(parserContext) {
    parserContext.unboundActions = {};
    parserContext.unboundFunctions = {};
    ...
    parserContext.entityToBoundActions = {};
    parserContext.entityToBoundFunctions = {};
 ...
 * 2. Unbound Functions can have collision; capture the max number of parameters.
 */
    ...
    for (let i = 0; i < elementDataList.length; i++) {
        ...
        if (isAction) {
            if (!parserContext.entityToBoundActions[boundEntityName]?.[elementName]) {
                ...
                throw new Error(messages.BOUND_ACTION_COLLISION + `${elementName} in entity ${entityName} of schema ${currNamespace}.`);
            }
        }
        else {
            if (!parserContext.entityToBoundFunctions[boundEntityName]?.[elementName]) {
                ...
                if (!functionObjList || !parserContext.entityToBoundFunctions[boundEntityName][elementName]) {
                    functionObjList = [];
                }
                ...
                parserContext.entityToBoundFunctions[boundEntityName][elementName] = functionObject;
            }
            // throw error if the return type is missing
