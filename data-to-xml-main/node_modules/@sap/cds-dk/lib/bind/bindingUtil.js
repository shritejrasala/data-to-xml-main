...
class BindingUtil {
    static getRequiresNameFromKind(service, kind) {
        ...
        if (!requiresName) throw new Error(`Unknown CDS service name for service ${bold(service)}. Please specify as argument for ${bold('cds bind')}.`)
        return requiresName;
    }
    ...
    static platformText(binding) {
        if (!PLATFORM_TYPES[binding.type]) throw new Error(`Unexpected binding type ${bold(binding.type)}.`);
        return PLATFORM_TYPES[binding.type]
    }

    static bindingText(binding) {
        switch (binding.type) {
            case 'cf':
                if (binding.key) {
