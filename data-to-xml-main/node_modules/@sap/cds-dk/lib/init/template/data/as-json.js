...
const asJson = module.exports = async (data, csn, numRecords=1, options={}) => {
  let { referenceData, contentType } = options
...
function dataForField(element, entity, csn, record, data, parentEntity, depth, referenceData) {
  ...
  if (element.is_struct) {
    ...
    for (const el of element.elements) {
      struct[el.name] = dataForField(el, entity, csn, record, data, null, depth+1, referenceData)
    }
    return struct
  }
  ...
  switch (type) {
    ...
    case 'cds.Composition': {
      ...
      if (!target || entity.name === target.name || depth > MAX_ASSOC_DEPTH) return
      ...
      for (let i = 0; i < targetRecordCount; i++) {
        const newRecord = fillRecord({}, targetRecordID+i, target, csn, data, entity, depth+1, referenceData)
    ...
    case 'cds.Association': {
      ...
      if (targetRecord) {
        ...
        if (targetRecords.length > 0) { // use record generated by following asociation
          ...
          if (parentEntity && (parentEntity.name != entity.name)) data[target.name].keepReferece = true
        }
      }
