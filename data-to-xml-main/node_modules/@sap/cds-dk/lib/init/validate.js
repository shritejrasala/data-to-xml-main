...
module.exports.projectFolder = cwd => {
  ...
  if (existingProjectFile) {
    let message
    if (cwd === cds.root) {
      message = `You seem to be working in a project which is already initialized. Use ${bold('cds add')} to add more features.`
    } else {
      message = `You seem to be trying to initialize an existing project. Use a different project name instead.`
    }
    throw `${message}\nDetails: File '${existingProjectFile}' exists in the project folder which indicates an existing project. Use ${bold('cds add')} to add more features.`
  }
}
