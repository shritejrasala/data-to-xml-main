...
class Linter {
  ...
  async lint(args, options) {
    ...
    const DefaultESLint = await this._loadEslint(currentDir);
    ...
    if (this.eslintCmdOpts.help) {
      ...
      try {
        await this._runEslint(currentDir, DefaultESLint);
      } catch (err) {
        term.error(err);
      }
    }
   ...
   */
  async _getConfigPathLegacy(currentDir = ".") {
    const configFiles = [
      ".eslintrc.js",
      ".eslintrc.cjs",
      ".eslintrc.yaml",
    ...
    while (configDir && configDir !== path.dirname(configDir)) {
      for (let i = 0; i < configFiles.length; i++) {
        ...
        const config = this.safeRequire(configPath)

        if (configFiles[i] !== "package.json" || config?.eslintConfig) {
 ...
 */
  async _getConfigPath(currentDir = ".") {
    ...
    while (configDir && configDir !== path.dirname(configDir)) {
      for (let i = 0; i < configFiles.length; i++) {
        ...
        const config = this.safeRequire(configPath)
        if (configFiles[i] !== "package.json" || config?.eslintConfig) {
          if (exists(configPath) && await isfile(configPath)) { // TODO exists neccessary ?
            return configPath;
          }
        }
      }
  ...
  async _runEslint(currentDir, DefaultESLint) {
    try {
      ...
      if (this.isESLintLegacy) {
        ...
        // Exvlude our own apis from plugin to avoid wrapper errors
  ...
  async _getConfigFileAndSetProjectPath(args) {
    ...
    this.configPath = this.isESLintLegacy
      ? await this._getConfigPathLegacy(process.cwd())
      : await this._getConfigPath(process.cwd());
    if (!this.configPath) {
      this.lintType === "global";
      this.configContents = this.isESLintLegacy
        ? await io.sanitizeEslintConfigLegacy({}, false, LOG)
        : await io.sanitizeEslintConfig({}, false, LOG);
    } else {
      const configContents = this.isESLintLegacy
        ? await io.readEslintConfigLegacy(this.configPath)
        : await io.readEslintConfig(this.configPath);
      ...
      if (this.isESLintLegacy) {
        ...
        const cdsConfig = configContents?.filter(c => c.files?.includes("*.cds"))[0] ?? {}
  ...
  async _overwriteRuleSeverities() {
    ...
    if (this.configPath) {
      ...
      configContents = this.isESLintLegacy
        ? await io.readEslintConfigLegacy(await this._getConfigPath(this.cdsdkPath))
        : await io.readEslintConfig(await this._getConfigPath(this.cdsdkPath));
    }
