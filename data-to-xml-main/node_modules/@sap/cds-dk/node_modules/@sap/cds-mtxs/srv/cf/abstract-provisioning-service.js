  ...
  async __upgrade(tenantIds, options) {
    ...
    if (isSync) {
      try {
        ...
        await this._sendCallback('FAILED', `Tenant upgrade failed with error '${error.message}'`)
        throw error
      }
    } else {
      const js = await cds.connect.to(JobsService)
      ...
      return tx.enqueue('cds.xt.DeploymentService', 'upgrade', dbToTenants, { options }, error => {
        if (error) this._sendCallback('FAILED', `Tenant upgrade failed with error '${error}'`)
