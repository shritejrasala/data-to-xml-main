...
if (typeof process.env.HDI_DEPLOY_OPTIONS !== 'undefined') {
  ...
  for (const option in options) {
    if (option === 'auto_undeploy') {
      ...
      args.translateJSONEnvStringOptionToOption(logger, options, name, option, process.argv);
    } else if (option === 'skipped_deleted_files_log') {
      args.translateJSONEnvStringOptionToOption(logger, options, name, option, process.argv);
...
for (let i = 2; i < process.argv.length; ++i) {
  ...
  if (arg === '-h' || arg === '--help') {
    const help = [
      ...
      '                                  [filepath] write skipped deleted files (that are not listed in undeploy.json) to the file indicated by the file path. The file must not exist;',
      '                                  [`local`] create a new file called skippedDeletedFiles.json in the deployer working directory and write the deleted files into it.',
      '                                  [`stdout`] write the deleted files to the standard output.',
  ...
  } else if (arg === '--skipped-deleted-files-log') {
    ++i;
    if (i >= process.argv.length) {
      logger.error('Option skipped-deleted-files-log requires a <file> argument or "stdout" or "local" as argument');
      process.exit(1);
    }
    opt.logSkippedDeletedFiles = process.argv[i];
    if (opt.logSkippedDeletedFiles !== 'stdout' && opt.logSkippedDeletedFiles !== 'local' && !path.isAbsolute(opt.logSkippedDeletedFiles)) {
      logger.error('Option skipped-deleted-files-log must be "local", "stdout", or an absolute file path.');
      process.exit(1);
    }
...
logger.setSkippedDeletedFileLogDestination(opt.logSkippedDeletedFiles);
